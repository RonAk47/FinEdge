...
describe('mock JWT', () => {
  ...
  it('signs a three-part token and verifies its claims', () => {
    ...
    expect(claims).toMatchObject({ sub: 'user-1', email: 'ahsas@example.com' });
  ...
  it('rejects a token signed with a different secret', () => {
    ...
    expect(() => verifyToken(`${header}.${payload}.forgedsignature`)).toThrow('Invalid token');
  });
...
describe('password hashing', () => {
  it('produces a salted hash that verifies only the right password', async () => {
    ...
    await expect(verifyPassword('sup3rsecret', passwordHash)).resolves.toBe(true);
  ...
  it('returns false for a malformed stored hash', async () => {
    await expect(verifyPassword('sup3rsecret', 'garbage')).resolves.toBe(false);
  });
});
